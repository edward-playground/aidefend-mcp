...
class TestVersionInfo:
    ...
    def test_version_info_structure(self):
        ...
        try:
            ...
            expected_keys = {'framework_version', 'last_sync_time', 'commit_sha'}

            # May not have all keys if never synced
            assert isinstance(version_info, dict)

        except FileNotFoundError:
            # Version file doesn't exist yet (never synced)
...
class TestStatusErrorHandling:
    ...
    def test_status_handles_missing_version_file(self):
        ...
        try:
            version_info = load_version_info()
            # Should return dict, possibly with missing keys
            assert isinstance(version_info, dict)
        except FileNotFoundError:
            # This is acceptable - version file may not exist
            pass
