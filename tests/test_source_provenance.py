...
def test_local_signature_uses_dynamic_manifest_membership_and_order(tmp_path, monkeypatch):
    ...
    assert first == sync_module._compute_staged_framework_digest(
        ...
        algorithm="sha1",
        source_files=["aidefend-intro.js", "alpha.js", "respond.js"],
    )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("source_kind", ["tactic", "intro", "manifest", "schema"])
async def test_github_download_preserves_immutable_response_bytes(
    ...
    if source_kind == "intro":
        ...
        downloaded = await sync_module.download_schema_metadata_file("a" * 40)
        expected_name = "data-schema.md"
    else:
